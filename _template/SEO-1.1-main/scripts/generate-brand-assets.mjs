...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'ark-cheats-logo.png'), logoPng);
	console.log('Wrote public/images/ark-cheats-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'ark-cheats-logo.webp'), logoWebp);
	console.log('Wrote public/images/ark-cheats-logo.webp');
}
